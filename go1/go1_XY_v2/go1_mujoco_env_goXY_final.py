...
class Go1MujocoEnv(MujocoEnv):
    ...
    def step(self, action):
        ...
        #terminated si se llega a las pos target a una distancia umbral(0.2)
        if np.linalg.norm(self.data.qpos[:2] - self._target_position) < self.distance_umbral:
            terminated=True
            reward += 150*self.reward_weights["termination"] 
    ...
    def heading_alignment_reward(self):
        ...
        if self._step > 300:
            return 0.0  # Ya no aporta después del paso N
        else:       
    ...
    def reward_velocity_towards_target(self):
        # Recompensa avanzar hacia objetivo
        ...
        forward_velocity = np.dot(velocity, direction)  # Componente hacia el objetivo
    ...
    def time_penalty(self):
        # Penaliza por paso tras N pasos
    ...
    def speed_penalty(self):
        # Penaliza si se supera la vel máxima
        speed = np.linalg.norm(self.data.qvel[:2])
        max_speed = self.max_velocity # m/s
        if speed > max_speed:
            return (speed - max_speed) ** 2
        else:
            return 0.0
    ...
    def _calc_reward(self, action):
        ...
        speed_penalty_cost = self.speed_penalty * self.cost_weights["speed_limit"]
        time_penalty_cost = self.time_penalty * self.cost_weights["time_penalty"]
    ...
    def _get_obs(self):
        ...
        desired_vel = self._desired_velocity.copy()
        last_action = self._last_action.copy()
        projected_gravity = self.projected_gravity

        #añadido por mi
        position=self.data.qpos.flatten()
        base_position_xy = position[:2]

        target_position_xy=self._target_position.copy()

        curr_obs = np.concatenate(
            (
                base_position_xy * self._obs_scale["base_position"],
                target_position_xy, 
    ...
    def reset_model(self):
        # Resetea las posiciones iniciale scon ruido
